...
def telegram_webhook():
    ...
    if tele_setting.expense_enabled == 1:
    ...
    if tele_setting.income_enabled == 1:
        """Webhook endpoint untuk Telegram income bukti bayar (tanpa Gemini OCR)."""
        # 1) Terima payload
        data = frappe.request.get_json() or {}
        msg  = data.get("message", {})
        ...
        if msg.get("photo"):
            ...
            # download dan load image (opsional, jika ingin validasi gambar)
            resp = requests.get(file_url)
            resp.raise_for_status()
            img = Image.open(BytesIO(resp.content))

            # Buat dokumen Telegram Income Log (field lain bisa diisi None/blank)
            income_log = frappe.get_doc({
                "doctype": "Telegram Income Log",
                "telegram_user": chat_id,
                "income_category": "Sales Invoice",
                "amount": None,
                "description": None,
                "user": frappe.db.get_value("Telegram User", {"telegram_user_id": chat_id}, "user") or frappe.session.user,
                "invoice": None,
            ...
            income_log.insert(ignore_permissions=True)

            # Attach file ke dokumen
            file_doc = frappe.get_doc({
                "doctype": "File",
                "file_name": file_url.split("/")[-1],
                "file_url": file_url,
                "attached_to_doctype": "Telegram Income Log",
                "attached_to_name": income_log.name
            }).insert(ignore_permissions=True)

            reply_text = (
                "Bukti bayar Anda sudah diterima dan akan segera diverifikasi. "
                "Terima kasih."
            )
        else:
            reply_text = "Silakan kirim foto bukti pembayaran."
